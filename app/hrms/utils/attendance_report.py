...
def check_leave_valid_type1(
    type_name, element, shift_start_datetime, shift_end_datetime
):
    result = False
    ...
    if c1 and c2:
        try:
            c3 = (
                not element["request_date_from"].replace(
                    hour=0, minute=0, second=0, microsecond=0
                )
                > shift_end_datetime
            )
            c4 = (
                not element["request_date_to"].replace(
...
def find_in_in_couple(list_attempt, scheduling_record):
    ...
    for item in list_attempt:
        if item.inout != InoutMode.Out:
            ...
            couple.atoffice_time = calculate_worktime_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt, scheduling_record
            )
            couple.nightWorkTime = calculate_night_worktime_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt, scheduling_record
            )
            couple.holidayWorkTime = calculate_holiday_worktime_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt, scheduling_record
            )
            couple.nightHolidayWorkTime = calculate_night_holiday_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt, scheduling_record
            )
...
def get_list_couple_out_in(list_couple_io, scheduling_record):
    ...
    if shift_start_datetime and shift_end_datetime and list_couple_io:
        if (
            shift_start_datetime < list_couple_io[0].itemIn.attempt
            and include_late_early
        ):
            couple = CoupleInout(
                itemIn=AttendanceAttemptInOut(attempt=shift_start_datetime),
                itemOut=AttendanceAttemptInOut(
                    attempt=list_couple_io[0].itemIn.attempt
                ),
            )
            couple.atoffice_time = calculate_worktime_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt
            )
            couple.nightWorkTime = calculate_night_worktime_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt
            )
        ...
        for i in range(1, len(list_couple_io)):
            couple = CoupleInout(
                itemIn=AttendanceAttemptInOut(
                    attempt=list_couple_io[i - 1].itemOut.attempt
                ),
                itemOut=AttendanceAttemptInOut(
                    attempt=list_couple_io[i].itemIn.attempt
                ),
            )
            couple.atoffice_time = calculate_worktime_without_inout(
                couple.itemIn.attempt, couple.itemOut.attempt
        ...
        if (
            shift_end_datetime > list_couple_io[-1].itemOut.attempt
            and include_late_early
        ):
            couple = CoupleInout(
                itemIn=AttendanceAttemptInOut(
                    attempt=list_couple_io[-1].itemOut.attempt
                ),
...
def kimode_worktime_without_inout(
    real_time_in,
    real_time_out,
    shift,
    kidmod,
    ...
    if shift and kidmod != "None":
        current_program = (
            real_time_out
            if real_time_out <= kid_mode_stage1_end_datetime
            else kid_mode_stage1_end_datetime
        ) - (
            real_time_in
            if real_time_in >= kid_mode_stage1_datetime
            else kid_mode_stage1_datetime
        )
        stage_first = max(0, current_program.total_seconds() // 60)

        current_program = (
            real_time_out
            if real_time_out <= kid_mode_stage2_end_datetime
            else kid_mode_stage2_end_datetime
        ) - (
            real_time_in
            if real_time_in >= kid_mode_stage2_datetime
            else kid_mode_stage2_datetime
        )
        ...
        result = [
            (
                ...
                if select_off_stage == 0
                else (stage_second if select_off_stage == 2 else stage_first)
            ),
...
def process_leave_item_ho(scheduling_record, leave_item):
    ...
    for item in attempt_with_inout_array:
        if (
            not item.attempt > leave_item["attendance_missing_to"]
            and not item.attempt < leave_item["attendance_missing_from"]
        ):
            item.inout = InoutMode.In

    if shift_start_datetime:
        if (
            not shift_start_datetime > leave_item["attendance_missing_to"]
            and not shift_start_datetime < leave_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(
                attempt=shift_start_datetime, inout=InoutMode.In
            )
            attempt_with_inout_array.append(item)

        if (
            rest_start_datetime
            and not rest_start_datetime > leave_item["attendance_missing_to"]
            and not rest_start_datetime < leave_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(
                attempt=rest_start_datetime, inout=InoutMode.Out
            )
            for in_out_add_item_before in [
                i for i in list_add_item_out if not i.attempt > item.attempt
            ]:
        ...
        if (
            shift_end_datetime
            and not shift_end_datetime > leave_item["attendance_missing_to"]
            and not shift_end_datetime < leave_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(
                attempt=shift_end_datetime, inout=InoutMode.Out
            )
            for in_out_add_item_before in [
                i for i in list_add_item_out if not i.attempt > item.attempt
            ]:
        ...
        if (
            rest_end_datetime
            and not rest_end_datetime > leave_item["attendance_missing_to"]
            and not rest_end_datetime < leave_item["attendance_missing_from"]
        ):
...
def process_explanation_item_ho(scheduling_record, explaination_item):
    ...
    for item in attempt_with_inout_array:
        if (
            not item.attempt > explaination_item["attendance_missing_to"]
            and not item.attempt < explaination_item["attendance_missing_from"]
        ):
            item.inout = InoutMode.In

    if shift_start_datetime:
        if (
            not shift_start_datetime > explaination_item["attendance_missing_to"]
            and not shift_start_datetime < explaination_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(
                attempt=shift_start_datetime, inout=InoutMode.In
            )
        ...
        if (
            rest_start_datetime
            and not rest_start_datetime > explaination_item["attendance_missing_to"]
            and not rest_start_datetime < explaination_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(
                attempt=rest_start_datetime, inout=InoutMode.Out
            )
        ...
        if (
            shift_end_datetime
            and not shift_end_datetime > explaination_item["attendance_missing_to"]
            and not shift_end_datetime < explaination_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(
                attempt=shift_end_datetime, inout=InoutMode.Out
            )
        ...
        if (
            rest_end_datetime
            and not rest_end_datetime > explaination_item["attendance_missing_to"]
            and not rest_end_datetime < explaination_item["attendance_missing_from"]
        ):
            item = AttendanceAttemptInOut(attempt=rest_end_datetime, inout=InoutMode.In)
            attempt_with_inout_array.append(item)


def calculate_night_worktime_without_inout(
    real_timein, real_timeout, scheduling_record
):
    ...
    if shift:
        ...
        if is_night_stage_fist:
            stageStart = (
                restStartDateTime
                if restStartDateTime < night_stage_fist_end
                else night_stage_fist_end
            )
            current_program = (
                real_timeout.replace(second=0)
                if real_timeout < stageStart
                else stageStart
            ) - (
                real_timein.replace(second=0)
                if real_timein > night_stage_fist_start
                else night_stage_fist_start
            )
            stage_fist = max(0, current_program.total_seconds() // 60)

            stageEnd = (
                restEndDateTime
                if restEndDateTime > night_stage_fist_start
                else night_stage_fist_start
            )
            current_program = (
                real_timeout.replace(second=0)
                if real_timeout < night_stage_fist_end
                else night_stage_fist_end
            ) - (real_timein.replace(second=0) if real_timein > stageEnd else stageEnd)
            stage_second = max(0, current_program.total_seconds() // 60)
        ...
        if is_night_stage_last:
            stageStart = (
                restStartDateTime
                if restStartDateTime < night_stage_last_end
                else night_stage_last_end
            )
            current_program = (
                real_timeout.replace(second=0)
                if real_timeout < stageStart
                ...
                real_timein.replace(second=0)
                if real_timein > night_stage_last_start
                else night_stage_last_start
            )
            stage_fist = max(0, current_program.total_seconds() // 60)

            stageEnd = (
                restEndDateTime
                if restEndDateTime > night_stage_last_start
                else night_stage_last_start
            )
            current_program = (
                real_timeout.replace(second=0)
                if real_timeout < night_stage_last_end
                else night_stage_last_end
            ) - (real_timein.replace(second=0) if real_timein > stageEnd else stageEnd)
            stage_second = max(0, current_program.total_seconds() // 60)
...
def calculate_worktime_without_inout(real_timein, real_timeout, scheduling_record):
    ...
    # if shift:
    currentProgram = (
        real_timeout if real_timeout < restStartDateTime else restStartDateTime
    ) - (real_timein if real_timein > shiftStartDateTime else shiftStartDateTime)
    stageFist = max(0, currentProgram.total_seconds() // 60)

    currentProgram = (
        real_timeout if real_timeout < shiftEndDateTime else shiftEndDateTime
    ) - (real_timein if real_timein > restEndDateTime else restEndDateTime)
...
def calculate_holiday_worktime_without_inout(
    real_timein, real_timeout, scheduling_record
):
    ...
    if (
        ...
        and is_holiday
        and (
            calculate_worktime_without_inout(
                holiday_start_datetime, holiday_end_datetime
            )
            > 0
            or "PH" in shift_name
        )
        ...
        stageStart = (
            restStartDateTime
            if restStartDateTime < holiday_end_datetime
            else holiday_end_datetime
        )
        current_program = (
            real_timeout.replace(second=0) if real_timeout < stageStart else stageStart
        ) - (
            real_timein.replace(second=0)
            if real_timein > holiday_start_datetime
            else holiday_start_datetime
        )
        stage_fist = max(0, int(current_program.total_seconds() // 60))

        stageEnd = (
            restEndDateTime
            if restEndDateTime > holiday_start_datetime
            else holiday_start_datetime
        )
        current_program = (
            real_timeout
            if real_timeout < holiday_end_datetime
            else holiday_end_datetime
        ) - (real_timein.replace(second=0) if real_timein > stageEnd else stageEnd)
...
def calculate_night_holiday_without_inout(real_timein, real_timeout, scheduling_record):
    holiday_night_stage_fist_start_datetime = scheduling_record[
        "holiday_night_stage_fist_start_datetime"
    ]
    holiday_night_stage_fist_end_datetime = scheduling_record[
        "holiday_night_stage_fist_end_datetime"
    ]
    holiday_night_stage_last_start_datetime = scheduling_record[
        "holiday_night_stage_last_start_datetime"
    ]
    holiday_night_stage_last_end_datetime = scheduling_record[
        "holiday_night_stage_last_end_datetime"
    ]
    restStartDateTime = scheduling_record["rest_start_datetime"]
    restEndDateTime = scheduling_record["rest_end_datetime"]
    shift = scheduling_record["shift"]
    ...
    if shift:
        ...
        if shift and holiday_night_stage_fist_end_datetime:
            stageStart = (
                restStartDateTime
                if restStartDateTime < holiday_night_stage_fist_end_datetime
                else holiday_night_stage_fist_end_datetime
            )
            currentProgram = (
                real_timeout.replace(second=0)
                if real_timeout < stageStart
                ...
                real_timein.replace(second=0)
                if real_timein > holiday_night_stage_fist_start_datetime
                else holiday_night_stage_fist_start_datetime
            )
            stageFist = max(0, int(currentProgram.total_seconds() // 60))

            stageEnd = (
                restEndDateTime
                if restEndDateTime > holiday_night_stage_fist_start_datetime
                else holiday_night_stage_fist_start_datetime
            )
            currentProgram = (
                real_timeout.replace(second=0)
                if real_timeout < holiday_night_stage_fist_end_datetime
                else holiday_night_stage_fist_end_datetime
            ) - (real_timein.replace(second=0) if real_timein > stageEnd else stageEnd)
        ...
        if shift and holiday_night_stage_last_end_datetime:
            stageStart = (
                restStartDateTime
                if restStartDateTime < holiday_night_stage_last_end_datetime
                else holiday_night_stage_last_end_datetime
            )
            currentProgram = (
                real_timeout.replace(second=0)
                if real_timeout < stageStart
                ...
                real_timein.replace(second=0)
                if real_timein > holiday_night_stage_last_start_datetime
                else holiday_night_stage_last_start_datetime
            )
            stageFist = max(0, int(currentProgram.total_seconds() // 60))

            stageEnd = (
                restEndDateTime
                if restEndDateTime > holiday_night_stage_last_start_datetime
                else holiday_night_stage_last_start_datetime
            )
            currentProgram = (
                real_timeout.replace(second=0)
                if real_timeout < holiday_night_stage_last_end_datetime
                else holiday_night_stage_last_end_datetime
            ) - (real_timein.replace(second=0) if real_timein > stageEnd else stageEnd)
            stageSecond = max(0, int(currentProgram.total_seconds() // 60))
...
def check_last_in_out(scheduling_record):
    ...
    if attempt_with_inout_array:
        if attempt_with_inout_array[-1].inout != InoutMode.Out:
            addItem = AttendanceAttemptInOut(
                attempt=attempt_with_inout_array[-1].attempt
            )
            addItem.inout = InoutMode.Out

            for in_out_addItem_before in [
                item
                for item in list_addItem_out
                if item.attempt <= attempt_with_inout_array[-1].attempt
            ]:
            ...
            add_in_item = AttendanceAttemptInOut(
                attempt=attempt_with_inout_array[-1].attempt
...
def calculate_night_worktime_custom(
    scheduling_record, real_timein, real_timeout, nightStageStart, nightStageEnd
):
    restStartDateTime = scheduling_record["rest_start_datetime"]
    restEndDateTime = scheduling_record["rest_end_datetime"]
    ...
    stageStart = (
        restStartDateTime if restStartDateTime < nightStageEnd else nightStageEnd
    )
    currentProgram = (
        real_timeout.replace(second=0) if real_timeout < stageStart else stageStart
    ) - (
        real_timein.replace(second=0)
        if real_timein > nightStageStart
        else nightStageStart
    )
    stageFist = max(0, currentProgram.total_seconds() // 60)
...
def get_list_late_in_leaves(scheduling_record):
    ...
    list_late_in_leaves = [
        element
        for element in _hrLeaves
        if check_leave_valid_type1(
            "đi muộn", element, shift_start_datetime, shift_end_datetime
        )
...
def get_list_early_out_leaves(scheduling_record):
    ...
    list_early_out_leaves = [
        ...
        for element in _hrLeaves
        if check_leave_valid_type1(
            "về sớm", element, shift_start_datetime, shift_end_datetime
        )
        # element['request_date_from']
        # and element['request_date_to']
        # and not element['request_date_from'].replace(hour=0, minute=0, second=0, microsecond=0) > shift_end_datetime
...
def check_is_night_stage_fist(scheduling_record):
    scheduling_record["is_night_stage_fist"] = (
        scheduling_record["night_stage_fist_start"]
        and scheduling_record["night_stage_fist_end"]
    )


def check_is_night_stage_last(scheduling_record):
    scheduling_record["is_night_stage_last"] = (
        scheduling_record["night_stage_last_start"]
        and scheduling_record["night_stage_last_end"]
    )
...
def calculate_night_stage_fist_start(scheduling_record):
    ...
    if date and shift:
        ...
        if (
            not fist_night_start > shift_end_datetime
            and not fist_night_end < shift_start_datetime
        ):
            result = (
                shift_start_datetime
                if fist_night_start < shift_start_datetime
                else fist_night_start
            )
...
def calculate_night_stage_fist_end(scheduling_record):
    ...
    if date and shift:
        ...
        if (
            ...
            result = (
                shift_end_datetime
                if fist_night_end > shift_end_datetime
                else fist_night_end
            )
...
def calculate_night_stage_last_start(scheduling_record):
    ...
    if date and shift:
        ...
        if (
            not last_night_start > shift_end_datetime
            and not last_night_end < shift_start_datetime
        ):
            result = (
                shift_start_datetime
                if last_night_start < shift_start_datetime
                else last_night_start
            )
...
def calculate_night_stage_last_end(scheduling_record):
    ...
    if date and shift:
        ...
        if (
            ...
            result = (
                shift_end_datetime
                if last_night_end > shift_end_datetime
                else last_night_end
            )
...
def calculate_holiday_night_stage_last_end_datetime(scheduling_record):
    ...
    try:
        ...
        if is_night_stage_last and is_holiday:
            if (
                not night_stage_last_start > holiday_end_datetime
                and not night_stage_last_end < holiday_start_datetime
            ):
                result = (
                    holiday_end_datetime
                    if night_stage_last_end > holiday_end_datetime
                    else night_stage_last_end
                )
    except Exception as ex:
    ...
    try:
        ...
        if is_night_stage_last and is_holiday:
            if (
                ...
                result = (
                    holiday_start_datetime
                    if night_stage_last_start < holiday_start_datetime
                    else night_stage_last_start
                )
    except Exception as ex:
...
def calculate_holiday_night_stage_fist_start_datetime(scheduling_record):
    ...
    try:
        ...
        if is_night_stage_fist and is_holiday:
            if (
                not night_stage_fist_start > holiday_end_datetime
                and not night_stage_fist_end < holiday_start_datetime
            ):
                result = (
                    holiday_start_datetime
                    if night_stage_fist_start < holiday_start_datetime
                    else night_stage_fist_start
                )
    except Exception as ex:
    ...
    try:
        ...
        if is_night_stage_fist and is_holiday:
            if (
                ...
                result = (
                    holiday_end_datetime
                    if night_stage_fist_end > holiday_end_datetime
                    else night_stage_fist_end
                )
    except Exception as ex:
...
def calculate_holiday_start_datetime(scheduling_record):
    ...
    try:
        ...
        if is_holiday and shift_start_datetime:
            min_datetime = min(holiday_calendar, key=lambda x: x["date_from"])[
                "date_from"
            ]
            result = (
                shift_start_datetime
                if shift_start_datetime > min_datetime
                else min_datetime
            )
    except Exception as ex:
...
def calculate_holiday_end_datetime(scheduling_record):
    ...
    try:
        ...
        if is_holiday and shift_end_datetime:
            ...
            result = (
                shift_end_datetime
                if shift_end_datetime < max_datetime
                else max_datetime
            )
    except Exception as ex:
    ...
    try:
        ...
        if date and shift_end_datetime and shift_start_datetime:
            for holiday in holiday_calendar:
                if (
                    not holiday["date_from"].replace(hour=0, minute=0, second=0)
                    > shift_end_datetime
                    and not holiday["date_to"].replace(hour=0, minute=0, second=0)
                    < shift_start_datetime
                ):
                    result = True
                    break
...
def process_leave_records(leave, scheduling_record):
    ...
    for leave_item in leave.leave_records:
        try:
            leave_item["holidayStatusName"] = (
                leave_item["holiday_status_id"][0]
                if leave_item["holiday_status_id"][1]
                else ""
            )
            if leave_item["attendance_missing_to"]:
                leave_item["attendance_missing_to"] = datetime.strptime(
                    leave_item["attendance_missing_to"], "%Y-%m-%d %H:%M:%S"
                )
            if leave_item["attendance_missing_from"]:
                leave_item["attendance_missing_from"] = datetime.strptime(
                    leave_item["attendance_missing_from"], "%Y-%m-%d %H:%M:%S"
                )
            if leave_item["request_date_from"]:
                leave_item["request_date_from"] = datetime.strptime(
                    leave_item["request_date_from"], "%Y-%m-%d"
                )
            if leave_item["request_date_to"]:
                leave_item["request_date_to"] = datetime.strptime(
                    leave_item["request_date_to"], "%Y-%m-%d"
                )
        except Exception as ex:
    ...
    for explanation_item in explanation.explaination_records:
        try:
            if explanation_item["attendance_missing_to"]:
                explanation_item["attendance_missing_to"] = datetime.strptime(
                    explanation_item["attendance_missing_to"], "%Y-%m-%d %H:%M:%S"
                )
            if explanation_item["attendance_missing_from"]:
                explanation_item["attendance_missing_from"] = datetime.strptime(
                    explanation_item["attendance_missing_from"], "%Y-%m-%d %H:%M:%S"
                )
        except Exception as ex:
        ...
        if scheduling["shift_name"].replace("/", "_") in merged_shift:
            ...
            if shift_name:
                if "/" in shift_name and "PH" in shift_name:
                    ...
                    temp_worktime_cal = calculate_worktime_without_inout(
                        main_shift_start_datetime, main_shift_end_datetime, scheduling
                    )
                    scheduling["half_stage1_worktime_calculate"] = (
                        calculate_worktime_without_inout(
                            main_shift_start_datetime,
                            main_rest_start_datetime,
                            scheduling,
                        )
                    )
                else:
                    temp_worktime_cal = calculate_worktime_without_inout(
                        shift_start_datetime, shift_end_datetime, scheduling
                    )
                    scheduling["half_stage1_worktime_calculate"] = (
                        calculate_worktime_without_inout(
                            shift_start_datetime, rest_start_datetime, scheduling
                        )
                    )
                ...
                if 370 <= temp_worktime_cal <= 375:
                    ...
                    scheduling["total_shift_worktime_calculate"] = (
                        round(temp_worktime_cal / 5) * 5
                    )
                if scheduling["main_contract"]:
                    scheduling["is_probationary"] = not (
                        "chính thức"
                        in f"{scheduling['main_contract']['contract_type_id']}".lower()
                    )


def add_attempt_more_than_limit(
    listAttendanceTrans, scheduling_record, diffHoursWithNext, diffHoursWithPrev
):
    attempt_with_inout_array = []
    attendanceAttemptArray = []
    if (
        scheduling_record["shift_start_datetime"]
        and scheduling_record["shift_end_datetime"]
    ):
        additionTrans = []
        ...
        listitemTrans = [
            e
            for e in listAttendanceTrans
            if shift_start - timedelta(hours=diffHoursWithPrev)
            < e["time_dt"]
            < shift_end + timedelta(hours=diffHoursWithNext)
        ]
        ...
        for tran in listitemTrans:
            ...
            additem.inout = (
                InoutMode.In
                if tran["in_out"] in ["I", "i"]
                else (
                    InoutMode.Out
                    if tran["in_out"] in ["O", "o"]
                    else InoutMode.NoneMode
                )
            )
        ...
        attendanceAttemptArray = list(
            set(
                [
                    e["time_dt"]
                    for e in listitemTrans
                    if e["time_dt"] not in attendanceAttemptArray
                    and e["time_dt"].replace(second=0) not in attendanceAttemptArray
                ]
                + attendanceAttemptArray
            )
        )
    ...
    scheduling_record["attendanceAttempt1"] = (
        attendanceAttemptArray[0] if len(attendanceAttemptArray) > 0 else None
    )
...
def process_missing_attendance(scheduling_record):
    ...
    for e in hrLeaves:
        e["holidayStatusName"] = (
            e["holiday_status_id"][0] if e["holiday_status_id"][1] else ""
        )

    # Lọc các phần tử có trạng thái thiếu chấm công
    listMissingLeaves = [
        element
        for element in hrLeaves
    ...
    if shiftStartDateTime:
        ...
        for leaveItem in listMissingLeaves:
            ...
            def add_to_attempt_array(attendance_time, mode):
                ...
                for in_out_addItem_before in [
                    item for item in list_addItem_out if item.attempt <= additem.attempt
                ]:
                    in_out_addItem_before.inout = InoutMode.NoneMode
            ...
            if (
                "chấm công ra" in f"{leaveItem['holidayStatusName']}".lower()
                and leaveItem.get("attendance_missing_to")
            ):
                attendance_missing_to = leaveItem["attendance_missing_to"]
                if attendance_missing_to.day in {
                    shiftStartDateTime.day,
                    shiftEndDateTime.day,
                }:
            ...
            if (
                "chấm công vào" in f"{leaveItem['holidayStatusName']}".lower()
                and leaveItem.get("attendance_missing_from")
            ):
                attendanceMissingFrom = leaveItem["attendance_missing_from"]
                if attendanceMissingFrom.day in {
                    shiftStartDateTime.day,
                    shiftEndDateTime.day,
                }:
                    if attendanceMissingFrom < (
                        shiftStartDateTime - timedelta(hours=3)
                    ):
                        attendanceMissingFrom += timedelta(days=1)
                    add_to_attempt_array(attendanceMissingFrom, InoutMode.In)

            if leaveItem.get("attendance_missing_to") and leaveItem.get(
                "attendance_missing_from"
            ):
                if {
                    leaveItem["attendance_missing_from"].day,
                    leaveItem["attendance_missing_to"].day,
                } & {shiftStartDateTime.day, shiftEndDateTime.day}:
...
def find_attendance_hue4_time_mode(scheduling_record):
    ...
    if restStartDateTime and restEndDateTime:
        for index in range(len(attendanceAttemptArray)):
            ...
            if attendanceAttemptArray[index] >= restStartDateTime:
                ...
                if (
                    ...
                    or (
                        attendanceAttemptArray[index]
                        - attendanceAttemptArray[afterRestStartEndIndex]
                    ).total_seconds()
                    > 1
                ):
        ...
        HueStage1Start = (
            attendanceAttemptArray[beforeRestEndStartIndex]
            if beforeRestEndStartIndex > -1
            else None
        )
        HueStage2End = (
            attendanceAttemptArray[afterRestStartEndIndex]
            if afterRestStartEndIndex > -1
            else None
        )

        if (
            beforeRestEndEndIndex > afterRestStartStartIndex
            and afterRestStartStartIndex >= 0
        ):
            HueStage1End = (
                attendanceAttemptArray[afterRestStartStartIndex]
                if afterRestStartStartIndex > -1
                else None
            )
            ...
            beforeRestEndEndIndex < afterRestStartStartIndex
            and beforeRestEndEndIndex >= 0
        ):
            HueStage1End = (
                attendanceAttemptArray[beforeRestEndEndIndex]
                if beforeRestEndEndIndex > -1
                else None
            )
            ...
            if beforeRestEndEndIndex >= 0 and HueStage1Start and HueStage2End:
                tempStart1End = next(
                    (
                        e
                        for e in reversed(attendanceAttemptArray)
                        if e > HueStage1Start
                        and e < attendanceAttemptArray[beforeRestEndEndIndex]
                    ),
                    None,
                )
                tempStart2Start = next(
                    (
                        e
                        for e in attendanceAttemptArray
                        if e < HueStage2End
                        and e > attendanceAttemptArray[beforeRestEndEndIndex]
                    ),
                    None,
    ...
    if HueStage2Start and HueStage2End and shiftEndDateTime and restEndDateTime:
        stage2WorktimeTemp = calculate_night_worktime_custom(
            scheduling_record,
            HueStage2Start,
            HueStage2End,
            restEndDateTime,
    ...
    if attendanceAttempt1:
            ...
            if "/OFF" not in shift_name and "OFF/" not in shift_name:
                if by_hue_shift:
                    ...
                    if HueStage1End and HueStage2Start:
                        ...
                        totalWorkTime = calculate_worktime_without_inout(
                            real_timein, real_timeout
                        )
                ...
                stage1Off = calculate_night_worktime_custom(
                    scheduling_record,
                    real_timein,
                    real_timeout,
                    shiftStartDateTime,
                ...
                stage2Off = calculate_night_worktime_custom(
                    scheduling_record,
                    real_timein,
                    real_timeout,
                    restEndDateTime,
        ...
        if totalShiftWorkTime_calculate > 0:
            if real_timein and shiftStartDateTime:
                real_late_in_minute = calculate_worktime_without_inout(
                    shiftStartDateTime, real_timein
                )
            if real_timeout and shiftEndDateTime:
                real_ealry_out_minute = calculate_worktime_without_inout(
                    real_timeout, shiftEndDateTime
                )

        nightWorkTime = calculate_night_worktime_without_inout(
            real_timein, real_timeout, scheduling_record
        )
        holidayWorkTime = calculate_holiday_worktime_without_inout(
            real_timein, real_timeout, scheduling_record
        )
        nightHolidayWorkTime = calculate_night_holiday_without_inout(
            real_timein, real_timeout, scheduling_record
        )
    else:
        totalWorkTime = 0
...
def process_late_early_leave(scheduling_record):
    ...
    employee_ho = (
        scheduling_record["main_info"]["employee_ho"]
        if "employee_ho" in scheduling_record["main_info"]
        else False
    )
    ...
    if real_timein and shiftStartDateTime:
            ...
            if employee_ho:
                if list_couple:
                    if kidmod == KidMode.SBEGIN30SEND30:
                        lateinTime = calculate_worktime_without_inout(
                            kidModeStage1EndDatetime,
                            list_couple[0].itemIn.attempt,
                            scheduling_record,
                        )
                        earlyOutTime = calculate_worktime_without_inout(
                            list_couple[-1].itemOut.attempt,
                            kidModeStage2Datetime,
                            scheduling_record,
                        )
                ...
                if kidmod == KidMode.SBEGIN30SEND30:
                    lateinTime = calculate_worktime_without_inout(
                        kidModeStage1EndDatetime, real_timein, scheduling_record
                    )
                    earlyOutTime = calculate_worktime_without_inout(
                        real_timeout, kidModeStage2Datetime, scheduling_record
                    )
                elif kidmod == KidMode.SBEGIN60:
                    lateinTime = calculate_worktime_without_inout(
                        kidModeStage1EndDatetime, real_timein
                    )
                    earlyOutTime = calculate_worktime_without_inout(
                        real_timeout, shiftEndDateTime, scheduling_record
                    )
                elif kidmod == KidMode.SEND60:
                    earlyOutTime = calculate_worktime_without_inout(
                        real_timeout, kidModeStage2Datetime, scheduling_record
                    ...
                    lateinTime = calculate_worktime_without_inout(
                        shiftStartDateTime, real_timein, scheduling_record
                    )
                else:
                    lateinTime = calculate_worktime_without_inout(
                        shiftStartDateTime, real_timein, scheduling_record
        ...
        if shift_name and shift:
            if "/OFF" in shift_name or "OFF/" in shift_name:
                ...
                earlyOutTime = (
                    earlyOutTime - 240 if earlyOutTime > 240 else earlyOutTime
                )
        ...
        for leaveItem in list_late_in_leaves:
            if real_timein > shiftStartDateTime:
                if leaveItem["for_reasons"] == "1":
                    lateIn_private += min(
                        maxLateEarly,
                        max(leaveItem["minutes"], leaveItem["time_minute"]),
                    )
        ...
        listEarlyOutLeave = [
            element
            for element in _hrLeaves
            if "về sớm" in f"{element['holidayStatusName']}".lower()
            and "đi muộn" not in f"{element['holidayStatusName']}".lower()
        ]
        for leaveItem in listEarlyOutLeave:
            if real_timeout < shiftEndDateTime:
                if leaveItem["for_reasons"] == "1":
                    earlyOut_private += min(
                        maxLateEarly,
                        max(leaveItem["minutes"], leaveItem["time_minute"]),
                    )
                else:
                    earlyOut_by_work += min(
                        maxLateEarly,
                        max(
                            earlyOutTime,
                            max(leaveItem["minutes"], leaveItem["time_minute"]),
...
def process_overtime_leave(scheduling_record):
    ...
    is_probationary = not (
        "chính thức"
        in f"{scheduling_record['main_contract']['contract_type_id']}".lower()
    )
    is_holiday = scheduling_record["is_holiday"]
    holiday_start_datetime = scheduling_record["holiday_start_datetime"]
    holiday_end_datetime = scheduling_record["holiday_end_datetime"]
    ...
    list_overtime_leaves = list(
        filter(lambda leave: is_overtime_leave(leave, shift_start_datetime), hr_leaves)
    )

    for leave_item in list_overtime_leaves:
        overtime_minutes_by_leave += max(
            leave_item["minutes"], leave_item["time_minute"]
        ) * max(0, leave_item["multiplier_work_time"])
        ...
        if "phát sinh tăng" in leave_item["reasons"].lower():
            total_increase_date += max(
                leave_item["minutes"], leave_item["time_minute"]
            ) * max(0, leave_item["multiplier_work_time"])
            if is_probationary:
                total_increase_probationary += max(
                    leave_item["minutes"], leave_item["time_minute"]
                ) * max(0, leave_item["multiplier_work_time"])
        else:
            overtime_by_leave += max(
                leave_item["minutes"], leave_item["time_minute"]
            ) * max(0, leave_item["multiplier_work_time"])
            if is_holiday and holiday_start_datetime:
                overtime_holiday_by_leave += max(
                    leave_item["minutes"], leave_item["time_minute"]
                ) * max(0, leave_item["multiplier_work_time"])
            if is_probationary:
                overtime_probationary += max(
                    leave_item["minutes"], leave_item["time_minute"]
                ) * max(0, leave_item["multiplier_work_time"])
                if is_holiday and (
                    calculate_worktime_without_inout(
                    ...
                    overtime_holiday_probationary_by_leave += max(
                        leave_item["minutes"], leave_item["time_minute"]
                    ) * max(0, leave_item["multiplier_work_time"])

    return (
        overtime_by_leave,
        overtime_holiday_by_leave,
        overtime_holiday_probationary_by_leave,
        overtime_probationary,
        overtime_minutes_by_leave,
        overtime_wage_by_leave,
        total_work_time,
        convert_overtime,
        total_increase_date,
...
def is_increase_leave(leave, shift_start_datetime, shift_end_datetime):
    ...
    c2 = True if leave.get("request_date_to") else False
    c3 = (
        not leave["request_date_from"].replace(hour=0, minute=0, second=0)
        > shift_end_datetime
    )
    c4 = (
        not leave["request_date_to"].replace(hour=23, minute=59, second=59)
        < shift_start_datetime
    )
...
def process_increase_leave(scheduling_record):
    ...
    shift_name = scheduling_record["shift_name"]
    minutes_per_day = (
        scheduling_record["main_contract"]["minutes_per_day"]
        if "minutes_per_day" in scheduling_record["main_contract"]
        else False
    )
    ...
    if not date or not shift_end_datetime or not shift_start_datetime:
        ...
        list_increase_leaves = list(
            filter(
                lambda leave: is_increase_leave(
                    leave, shift_start_datetime, shift_end_datetime
                ),
                hr_leaves,
            )
        )
...
def process_leave_with_pay(scheduling_record):
    ...
    if not date or not shift_end_datetime or not shift_start_datetime:
        ...
        list_paided_leaves = list(
            filter(
                lambda leave: is_paid_leave(
                    leave, shift_start_datetime, shift_end_datetime
                ),
                hr_leaves,
            )
...
def process_annual_leave(scheduling_record):
    ...
    shift_start_datetime = scheduling_record["shift_start_datetime"]
    shift_end_datetime = scheduling_record["shift_end_datetime"]
    scheduling_record["list_al_leaves"] = list(
        filter(
            lambda leave: check_leave_valid_type1(
                "nghỉ phép năm", leave, shift_start_datetime, shift_end_datetime
            ),
            hr_leaves,
        )
    )
...
def process_casual_leave(scheduling_record):
    ...
    shift_start_datetime = scheduling_record["shift_start_datetime"]
    shift_end_datetime = scheduling_record["shift_end_datetime"]
    scheduling_record["list_cl_leaves"] = list(
        filter(
            lambda leave: check_leave_valid_type1(
                "nghỉ bù", leave, shift_start_datetime, shift_end_datetime
            ),
            hr_leaves,
        )
    )
...
def process_worktime_ho(scheduling_record):
    ...
    rest_end_datetime = scheduling_record["rest_end_datetime"]
    # rest_start_datetime = scheduling_record['rest_start_datetime']
    list_couple_after_explanation_private = scheduling_record[
    ...
    if "attendanceAttempt1" in scheduling_record:
        list_couple_before_explanation = find_in_out_couple(
            attempt_with_inout_array, scheduling_record
        )
        scheduling_record["list_couple_in_in_before_explanation"] = find_in_in_couple(
            attempt_with_inout_array, scheduling_record
        )
        scheduling_record["list_couple_out_in_before_explanation"] = (
            get_list_couple_out_in(list_couple_before_explanation, scheduling_record)
        )

    if date and shift_start_datetime and shift_end_datetime:
        for explaination_item in [
            e
            for e in list_explanations
            if e["reason"] == "2"
            and e["attendance_missing_from"]
            and e["attendance_missing_to"]
            and (
                (
                    e["attendance_missing_from"].day == date.day
                    and e["attendance_missing_from"].month == date.month
                )
                or (
                    e["attendance_missing_to"].day == date.day
                    and e["attendance_missing_to"].month == date.month
                )
            )
        ]:
    ...
    list_workingout_leaves = [
        element
        for element in hr_leaves
        if check_leave_valid_type2("ra ngoài", element, date)
    ]

    for leave_item in [
        element for element in list_workingout_leaves if element["for_reasons"] == "2"
    ]:
        process_leave_item_ho(
            leave_item,
            attempt_with_inout_array,
            shift_start_datetime,
            rest_start_datetime,
    ...
    for leave_item in [
        element
        for element in list_late_in_leaves
        if element["for_reasons"] == "2"
        and element["request_date_from"]
        and element["request_date_from"].day != shift_start_datetime.day
    ]:
        leave_item["attendance_missing_from"] = shift_start_datetime
        leave_item["attendance_missing_to"] = shift_start_datetime + timedelta(
            minutes=leave_item.minutes
        )
        if rest_start_datetime and rest_end_datetime:
            if leave_item["attendance_missing_to"] > rest_start_datetime:
                leave_item["attendance_missing_to"] += (
                    rest_end_datetime - rest_start_datetime
                )
        process_leave_item_ho(
            leave_item,
            attempt_with_inout_array,
    ...
    check_last_in_out(scheduling_record)
    if "attendanceAttempt1" in scheduling_record:
        list_couple_before_explanation_private = find_in_out_couple(
            attempt_with_inout_array, scheduling_record
        )
        scheduling_record["list_couple_out_in_before_explanation_private"] = (
            get_list_couple_out_in(
                list_couple_before_explanation_private, scheduling_record
            )
        )
        scheduling_record["list_couple_before_explanation_private"] = (
            list_couple_before_explanation_private
        )
        if list_couple_before_explanation_private:
            real_timein_couple = next(
                (
                    element
                    for element in list_couple_before_explanation_private
                    if element.itemOut.attempt > shift_start_datetime
                ),
                None,
            )
            real_timein = (
                real_timein_couple.itemIn.attempt if real_timein_couple else None
            )

            real_timeout_couple = next(
                (
                    element
                    for element in reversed(list_couple_before_explanation_private)
                    if element.itemIn.attempt < shift_end_datetime
                ),
                None,
            )
            ...
            if real_timeout_couple:
                real_timeout = (
                    real_timein
                    if real_timein_couple is None
                    else real_timeout_couple.itemOut.attempt
                )
    ...
    if "attendanceAttempt1" in scheduling_record:
        list_couple_after_explanation_private = find_in_out_couple(
            attempt_with_inout_array, scheduling_record
        )
        scheduling_record["list_couple_out_in_after_explanation_private"] = (
            get_list_couple_out_in(
                list_couple_after_explanation_private, scheduling_record
            )
        )
        scheduling_record["list_couple_after_explanation_private"] = (
            list_couple_after_explanation_private
        )

    for leave_item in [
        element
        for element in list_workingout_leaves
        if element["for_reasons"] == "1"
        and element["attendance_missing_from"]
        and element["attendance_missing_to"]
    ]:
        process_leave_item_ho(scheduling_record, leave_item)

    scheduling_record["list_couple"] = find_in_out_couple(
        attempt_with_inout_array, scheduling_record
    )
    scheduling_record["list_couple_out_in"] = get_list_couple_out_in(
        scheduling_record["list_couple"], scheduling_record
    )
...
def process_business_leave(scheduling_record):
    ...
    shift_start_datetime = scheduling_record["shift_start_datetime"]
    shift_end_datetime = scheduling_record["shift_end_datetime"]
    scheduling_record["list_business_leaves"] = list(
        filter(
            lambda leave: check_leave_valid_type1(
                "công tác", leave, shift_start_datetime, shift_end_datetime
            ),
            hr_leaves,
        )
    )
...
def process_child_mode(
    ...
    employee_ho,
    list_couple,
    out_by_private_attendance,
    out_by_work_attendance,
    select_off_stage,
    kidmod,
    kidmode_worktime_without_inout,
    kidmod_early_out_mid,
    kidmod_late_in_mid,
):
    kidmod_work_time = 0
    ...
    if attendance_attempt1 and real_time_in and real_time_out:
        if employee_ho:
            ...
            for couple in list_couple:
                kidmode_tuple = kidmode_worktime_without_inout(
                    couple.itemIn.attempt, couple.itemOut.attempt
                )
...
def process_explanation(
    ...
    listexplainations_private = [
        element for element in list_explanations if element["reason"] == "1"
    ]

    for explaination_item in [
        element
        for element in listexplainations_private
        if element["attendance_missing_from"] and element["attendance_missing_to"]
    ]:
        if employee_ho:
            in_time_leave = 0
            ...
            if real_time_in and real_time_out:
                if kidmod == "None":
                    ...
                    kid_mode_stage1_datetime
                    and kid_mode_stage1_end_datetime
                    and kid_mode_stage2_datetime
                    and kid_mode_stage2_end_datetime
                ):
    ...
    listexplainations_work = [
        element for element in list_explanations if element["reason"] == "2"
    ]

    for explaination_item in [
        element
        for element in listexplainations_work
        if element["attendance_missing_from"] and element["attendance_missing_to"]
    ]:
        in_time_leave = 0
        for couple in list_couple_before_explanation_private:
...
def process_working_out_leave(
    ...
    list_workingout_leaves = [
        ...
        if (
            ...
            and (
                ...
                or (
                    element["attendance_missing_to"].day == date.day
    ...
    for leave_item in list_workingout_leaves:
        out_time = calculate_worktime_without_inout(
            leave_item["attendance_missing_from"], leave_item["attendance_missing_to"]
        )
...
def process_working_out_leave_ho(
    hr_leaves,
    scheduling_record,
    date,
    list_couple_before_explanation_private,
...
def process_personal_holiday(
    ...
    shift,
    minutes_per_day,
    holiday_work_time_final,
    employee_ho,
    total_work_time_final,
):
    ph_date = 0.0
    ...
    if (
        ...
        if shift_name.startswith("PH") and "/" not in shift_name:
            ...
            half_ph_worktime_part = round(
                (1 - half_ph_leave) * (minutes_per_day if employee_ho else 480)
            )
            ph_date = min(1, half_ph_leave)
...
def process_up_shift(
    shift, shift_name, list_up_leaves, max_late_early, employee_ho, minutes_per_day
):
    total_up = 0.0
    up_by_leave = 0
